...
#   Middle (5,000–8,500 m, r=1,830 m) — transported NNE
#   Upper  (8,500–16,000 m, r=3,050 m) — transported NE at 50 mph
...
#
# Usage:
#   julia --threads=12 --project=../.. smoky_cmaes_particle_size.jl OU
#
...
const LB = Float64[
    ...
    1500.0, 3000.0, 6000.0                 # layer heights (m AGL): stem_top, cap_mid, cloud_top
]
...
# Warm start fallback — DASA-1251 Smoky cloud geometry converted to AGL:
# MSL values: stem top 3231 m, cap mid 6950 m, cloud top 10668 m
# NTS surface elevation: 1409 m MSL → AGL = MSL - 1409
const WARM_START_PARAMS = clamp.([
    127.552, 2.669, 141.861, 2.523, 0.8652,   # particle size (from Nancy OU best)
    ...
    1822.0, 5541.0, 9259.0                     # layer heights AGL from DASA-1251 (MSL - 1409m surface)
], LB, UB)
...
const ERA5_DIR = joinpath(@__DIR__, "ERA5_data")
const ERA5_FILES = if isdir(ERA5_DIR) && !isempty(filter(f -> endswith(f, "_snap.nc"), readdir(ERA5_DIR)))
...
const START_DT = Dates.DateTime(1957, 8, 31, 12, 0)
...
const RELEASE_X, RELEASE_Y = Transport.latlon_to_grid(DOMAIN, 37.177, -116.046)
println("   Release: grid ($(round(RELEASE_X, digits=1)), $(round(RELEASE_Y, digits=1)))")

println("\n3. Loading Smoky observations...")
const SMOKY_OBS = Transport.load_smoky_observations()
const LAT_GRID, LON_GRID = Transport.suggest_grid(SMOKY_OBS; resolution_km=2.0, buffer_fraction=0.5)
const OBS_MASKS = Transport.rasterise_all_contours(SMOKY_OBS.dose_rate_contours, LAT_GRID, LON_GRID)
...
const SOURCE_LAT = 37.177
const SOURCE_LON = -116.046
...
function load_checkpoint_params(turb_suffix::String)
    ckpt = joinpath(@__DIR__, "smoky_cmaes_$(turb_suffix)_best.txt")
...
x0 = if USE_WARM_START
    ...
    if ckpt_params !== nothing
        println("\n   Warm start from checkpoint: smoky_cmaes_$(turb_suffix)_best.txt")
        ckpt_params
    else
...
results_file = joinpath(@__DIR__, "smoky_cmaes_$(lowercase(string(TURB_SCHEME)))_results.txt")
checkpoint_file = joinpath(@__DIR__, "smoky_cmaes_$(lowercase(string(TURB_SCHEME)))_best.txt")
...
open(results_file, "w") do f
    println(f, "Smoky BIPOP-CMA-ES Results — $(TURB_NAME)")
    println(f, "="^60)
