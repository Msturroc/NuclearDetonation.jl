...
function run_dispersion_simulation(;
    ...
    if release_mode == "npp"
        ...
        pos_s, act_s, released_s = Transport.generate_release_particles(
            ...
            ones(Float64, era5.ny_met, era5.ny_met),
        ...
        Transport.run_simulation!(state, era5.files,
    ...
    # Convert to dose rate (mR/h at H+duration)
    dlat = abs(lat_grid[2] - lat_grid[1])
    dlon = abs(lon_grid[2] - lon_grid[1])
    ref_lat = 0.5 * (first(lat_grid) + last(lat_grid))
    dy_m = dlat * 111_000.0
    ...
    mSv_to_mR = 100.0
    dose_factor = K_DOSE * decay_factor * mSv_to_mR / cell_area_m2
    ...
    return SimulationResult(dose_smooth, lon_grid, lat_grid, max_dose,
                            state.deposition_log, p.smooth_sigma, domain, "mR/h")
end
